[claytonfarr.github.io /ralph-playbook/](https://claytonfarr.github.io/ralph-playbook/)
...
**Key files:** `PROMPT.md` (instructions) + `AGENTS.md` (operational guide) + `IMPLEMENTATION_PLAN.md` (task list) + `specs/*` (requirements)
...
![Geoff Huntley responding 'nah' to various Ralph interpretations](https://raw.githubusercontent.com/ClaytonFarr/ralph-playbook/main/references/nah.png)
...
[![Ralph workflow diagram showing the three phases](https://raw.githubusercontent.com/ClaytonFarr/ralph-playbook/main/references/ralph-diagram.png)](https://raw.githubusercontent.com/ClaytonFarr/ralph-playbook/main/references/ralph-diagram.png)
...
No plan exists, or plan is stale/wrong
...
Requirements doc for one topic of concern (`specs/FILENAME.md`)

_Task_

Unit of work derived from comparing specs to code
...
*   _Use protection_
    ...
    *   Options: Docker sandboxes (local), Fly Sprites/E2B/etc. (remote/production) - [additional notes](https://claytonfarr.github.io/ralph-playbook/references/sandbox-environments.md)
...
2.  **PROMPT.md instructs** → "Study IMPLEMENTATION\_PLAN.md and choose the most important thing..."
3.  **Agent completes one task** → updates IMPLEMENTATION\_PLAN.md on disk, commits, exits
4.  **Bash loop restarts immediately** → fresh context window
5.  **Agent reads updated plan** → picks next most important thing...

Key insight

The IMPLEMENTATION\_PLAN.md file persists on disk between iterations and acts as shared state between otherwise isolated loop executions. Each iteration deterministically loads the same files (`PROMPT.md` + `AGENTS.md` + `specs/*`) and reads the current state from disk.
...
*   Each iteration = one fresh context window = one task from IMPLEMENTATION\_PLAN.md = one commit
...
_Files:_ [`loop_streamed.sh`](https://claytonfarr.github.io/ralph-playbook/files/loop_streamed.sh) · [`parse_stream.js`](https://claytonfarr.github.io/ralph-playbook/files/parse_stream.js)
...
Task selection loop orchestration
...
Main instructions: task, validation, commit
...
One agent, one task, one loop at a time
...
Ralph still prioritizes and chooses implementation approach

Plan is disposable

✅ Yes

Wrong test requirements? Regenerate plan
...
_Test requirements_ (in plan) = Verification points derived from acceptance criteria
...
Criteria in plan (deterministic), evaluation non-deterministic but converges through iteration. Intentional tradeoff for subjective quality.
...
Ralph discovers pattern, chooses when to use, writes criteria
...
*   ✅ _Single plan per branch_ - one IMPLEMENTATION\_PLAN.md per branch
...
1.  _Full Planning (on main branch)_
    
        ./loop.sh plan
        # Generate full IMPLEMENTATION_PLAN.md for entire project
    
2.  _Create Work Branch_
...
3.  _Scoped Planning (on work branch)_
...
Scoping at plan creation (deterministic), not runtime (prob.)

Simple

✅ Yes

Optional enhancement, main workflow still works

Plan-driven

✅ Yes

One IMPLEMENTATION\_PLAN.md per branch

Single source of truth

✅ Yes

One plan per branch – scoped plan replaces full plan on branch

Plan is disposable

✅ Yes

Regenerate scoped plan anytime: `./loop.sh plan-work "work description"`

Markdown over JSON

✅ Yes

Still markdown plans

Let Ralph Ralph

✅ Yes

Ralph picks "most important" from already-scoped plan – no filter
...
The story map gives you _structure_ for slicing. Jason Cohen's [Simple, Lovable, Complete (SLC)](https://longform.asmartbear.com/slc/) gives you _criteria_ for what makes a slice good:
...
_III. Building Phase:_ Performed in Ralph loop with standard building prompt.
...
_Files:_ [`PROMPT_specs.md`](https://claytonfarr.github.io/ralph-playbook/files/PROMPT_specs.md)
...
*   **Bugs become specs:** The prompt intentionally documents buggy behavior as the defined behavior. Reverse-engineered specs describe what _is_, not what _should be_. Write new specs separately for desired behavior changes.
...
Specs are written artifacts (known state), not ad-hoc context, contains all flaws in code.

Context efficiency

⚠️ Partial

Must be adopted throughout your entire team culture

Capture the why

⚠️ Partial

Not all implemented code contains the why behind things, only captures comments if they express the why intention.

Let Ralph Ralph

✅ Yes

Topics of concern are still chosen by Ralph.

Plan is disposable

✅ Yes

Specs provide stable baseline; plans regenerate against documented reality

Simplicity wins

✅ Yes

Provides a Hawkeye view of your entire specifications.
...
_Files:_ [`PROMPT_reverse_engineer_specs.md`](https://claytonfarr.github.io/ralph-playbook/files/PROMPT_reverse_engineer_specs.md)
